...
import org.teutinc.pi.camerabox.CameraBox;
import org.teutinc.pi.camerabox.activity.runner.State;
...
public class WebSocketModule {
    ...
    @Provides @Named("Supplier<CameraBoxWebSocket>")
    ...
    static class WebSocketDispatcher implements AutoPreparable {
        ...
        private final List<CameraBoxWebSocket> sockets = new CopyOnWriteArrayList<>();
        ...
        public void prepare() {
            eventBus.register(new Object() {
                ...
                public void onWatchEvent(State event) {
                    final Iterator<CameraBoxWebSocket> iterator = sockets.iterator();
                    if (iterator.hasNext()) {
                        try {
        ...
        public WebSocket.OnTextMessage newWebSocket() {
            return new CameraBoxWebSocket();
        }

        private void register(CameraBoxWebSocket webSocket) {
            sockets.add(webSocket);
        }

        private void unregister(CameraBoxWebSocket webSocket) {
        ...
        private class CameraBoxWebSocket implements WebSocket.OnTextMessage {
